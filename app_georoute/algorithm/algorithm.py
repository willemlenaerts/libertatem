...
GOOGLE_MAPS_API_KEY = "AIzaSyCHDhf5OLJB_VVKZI0b6bfQ7LZiGxeivaA"
waypoints_file = "route/algorithm/data/my-waypoints-dist-dur.tsv"
...
# from route.algorithm.data.countries import countries
# import csv
# with open('route/algorithm/data/lijst_vlaamse_gemeenten.csv', 'r') as csvfile:
#     gemeenten_csv = csv.reader(csvfile)
#     gemeenten = []
#     for rij in gemeenten_csv:
#         gemeenten.append(rij[0] + ", België")

# all_waypoints = gemeenten
# # Capitals of EU
# from route.algorithm.data.countries import countries

# # Make waypoints from countries list
# all_waypoints = []
# for country in countries:
#     if country["continent"] == "Europe":
#         all_waypoints.append(country["capital"]) 
...
# if 5==3: #os.path.exists(file_path)
...
#             if count == len(all_waypoints):
#                 stop = time.time()
#                 print("For one waypoint the time to get distances is " + str(round(stop-start)) + " seconds.")
#                 print("The total remaining time for the other waypoints is " + str((len(all_waypoints)-1)*round(stop-start)) + " seconds.")
...
#         out_file.write(bytes("\t".join(["waypoint1",
#                                   "waypoint2",
#                                   "distance_m",
#                                   "duration_s"]), 'UTF-8'))
...
#                                       str(waypoint_durations[frozenset([waypoint1, waypoint2])])]),'UTF-8'))

# import waypoint data using pickle
import pickle
waypoint_distances = pickle.load(open("route/algorithm/data/waypoint_distances.p", "rb"))
waypoint_durations = pickle.load(open("route/algorithm/data/waypoint_durations.p", "rb"))
all_waypoints = pickle.load(open("route/algorithm/data/all_waypoints.p", "rb"))
