...
def get_pi_learning_rates(games,country,years_to_develop_ratings,rule="Goal Difference"):
    ...
    games = games[games.Competition == country].reset_index(drop=True).sort("Date")
    ...
    if rule == 'Profit': # HIGHEST is the best
        l_output = output.sort(rule,ascending = False).iloc[0].l
        g_output = output.sort(rule,ascending = False).iloc[0].g
    elif rule == "Goal Difference": # LOWEST is the best
        l_output = output.sort(rule,ascending = True).iloc[0].l
        g_output = output.sort(rule,ascending = True).iloc[0].g
