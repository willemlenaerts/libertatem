...
panda = panda[(panda.FTHG != "") & (panda.FTHG.str.len() <= 2) & (panda.FTAG.str.len() <= 2)]                                                                                                                                                                                                                                            

# Remove where FTHG of FTAG == ''
...
for year in years:
    for i in range(len(date_index[year])):
        date_index_reversed[date_index[year][i]] = year
...
for counter,country in enumerate(countries):
    ...
    country_table.append({ "country":country,
    ...
    for year in years:
        ...
        data[country][year].append(date_index[year])
        ...
        for i in range(len(date_index[year])):
            try:
                data[country][year][-1].append(dataframe_elo_list[i])
            except:
        ...
        try:
            max_elo[country].append(max(list(filter(None,data[country][year][-1]))))
        except:
        ...
        # Get minimum elo
        try:
            min_elo[country].append(min(list(filter(None,data[country][year][-1]))))
        except:
            # Dit seizoen speelde ploeg niet
        ...
        for i in range(len(date_index[year])):
            try:
                data[country][year][-1].append(dataframe_wedstrijd_list[i])
            except:
                data[country][year][-1].append(None)
        ...
        for i in range(len(date_index[year])):
            try:
                data[country][year][-1].append(dataframe_type_list[i])
            except:
                data[country][year][-1].append(None)        
...
json.dump(date_index,open("app_voetbalelo/wereld_geschiedenis/algorithm/results/json.dump/date_index.json","w"))
json.dump(date_index_reversed,open("app_voetbalelo/wereld_geschiedenis/algorithm/results/json.dump/date_index_reversed.json","w"))
